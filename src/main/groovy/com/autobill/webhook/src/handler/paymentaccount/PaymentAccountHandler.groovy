...
class PaymentAccountHandler extends AutoBillHandler {
    @Override
    List<Instance> determineInstance(CachedBodyHttpServletRequest httpServletRequest, HttpSession httpSession, Cookie[] cookies) {
        ...
        List<IntegrationProperty> integrationProperties = IntegrationProperty.findAllByApplication(IntegrationApplication.findByName(Constants.APPLICATIONS.PAYMENT_ACCOUNT))
        for (IntegrationProperty integrationProperty : integrationProperties ) {
            String identifier = httpServletRequest.getParameter(Constants.REQUEST_IDENTIFIER.PAYMENT_ACCOUNT)

            if (JSON.parse(integrationProperty.data)[Constants.REQUEST_IDENTIFIER.PAYMENT_ACCOUNT].toString() == identifier) {
    ...
    boolean matchProperty(IntegrationProperty integrationProperty, JSONElement jsonElement) {
        return JSON.parse(integrationProperty.data)[Constants.REQUEST_IDENTIFIER.PAYMENT_ACCOUNT].toString() == jsonElement[Constants.REQUEST_IDENTIFIER.PAYMENT_ACCOUNT].toString()
    }
}
